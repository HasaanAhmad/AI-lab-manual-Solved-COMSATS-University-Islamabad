...
# of improvement in that neighbor) whether to move to that neighbor or to examine another neighbor.
def hilclimbing(graph, start, goal):
    current = start
    while current != goal:
        neighbor = graph[current].actions[0][0]
        if graph[neighbor].heuristic < graph[current].heuristic:
            current = neighbor
        else:
            break
    return current
